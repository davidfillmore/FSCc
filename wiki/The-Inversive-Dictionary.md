...
disproving the Jacobian conjecture for all $`n\ge3`$ [Alpöge]; the case $`n=2`$
remains open. Both displayed facts are certified (checks
...
![The power of a point with respect to a cycle](https://raw.githubusercontent.com/wiki/davidfillmore/FSCc/images/power-of-a-point.svg)

*Figure 1. Kisil's elliptic model draws a point-pair on the line as the semicircle on it. Left: the power of a point is the squared tangent length, the value of the quadratic at the point. Middle: the zero-radius limit, the point-cycle of a root. Right: Tao's stratum, the pair at 0 and 3/2 with the point at 2, where the power is exactly 1. Generated by `tools/make-figures.py` in the repository.*
...
![A triple splits three ways](https://raw.githubusercontent.com/wiki/davidfillmore/FSCc/images/three-to-one.svg)

*Figure 3. The triple −1, 1/2, 2 and its three splittings into a point and a pair, each pair drawn as its semicircle. A split determines the pair (L, Q) up to the scaling (λL, λ⁻¹Q), which multiplies the resultant by λ, so exactly one scaling lands on the stratum Res = 1: the three preimages of a generic cubic.*
